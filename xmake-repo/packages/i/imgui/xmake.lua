package("imgui")
    ...
    add_configs("docking", {description = "Use the docking branch", default = false, type = "boolean"})
    ...
    on_load("macosx", "linux", "windows", "mingw", "android", "iphoneos", function (package)
        ...
        if package:config("docking") then
            package:set("urls", {"https://github.com/ocornut/imgui.git"})
            local versions = {}
            versions["v1.83"] = "80b5fb51edba2fd3dea76ec3e88153e2492243d1"

            package:set("versions", versions)
            print(package:get("versions"))
        end
    end)
